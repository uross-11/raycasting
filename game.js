...
function renderFloorAndCeiling(ctx, player, backImageData) {
    ...
    for (let y = 0; y < height; y++) {
        ...
        for (let x = 0; x < width; x++) {
            ...
            // const glow = Math.min((1 / zBuffer[x]) * 4, 1);
...
export class Game {
    ...
    handleMessage(data) {
        switch (data.type) {
            ...
            case "update":
                ...
                this.visibleSprites = [
                    { pos: [21.5, 2.5], imgData: this.assets.get("bomb"), tag: "enemy" },
                    ...Array.from(this.players.values())
                        .filter((player) => player.id !== this.id) // Exclude self
                        .map((player) => ({
                            pos: player.pos,
                            imgData: this.assets.get("barrel"), // or create a player sprite
                            tag: "player",
                        })),
                ];
                break;
    ...
    render(ctx, backCtx, deltaTime) {
        ...
        resetSpritePool(this.spritePool, this.visibleSprites, this.players, this.me);
        this.backImageData.data.fill(0);
        ...
        if (this.mapShown) {
            drawMap(ctx, common.map);
            drawPlayersOnMap(ctx, this.players);
            drawSpritesOnMap(ctx, this.visibleSprites);
        }
